...
extension NETunnelProviderProtocol {
    convenience init?(tunnelConfiguration: TunnelConfiguration, previouslyFrom old: NEVPNProtocol? = nil) {
        ...
        providerBundleIdentifier = "com.turisvpn.app.connect"
    ...
    func migrateConfigurationIfNeeded(called name: String) -> Bool {
        ...
        if let oldConfig = providerConfiguration?["WgQuickConfig"] as? String {
            ...
            print("Migrating tunnel configuration '\(name)'")
        ...
        if passwordReference != nil && passwordReference!.count == 12 {
            ...
            if !newReference.elementsEqual(passwordReference!) {
                print("Migrating iOS 14-style keychain reference to iOS 15-style keychain reference for '\(name)'")
