...
protocol TunnelsManagerActivationDelegate: AnyObject {
    ...
    func tunnelActivationFailed(tunnel: TunnelContainer) // status didn't change to connected
...
class TunnelsManager {
    ...
    static func create(completionHandler: @escaping (Result<TunnelsManager, TunnelsManagerError>) -> Void) {
        NETunnelProviderManager.loadAllFromPreferences { managers, error in
            if let error = error {
                print("Failed to load tunnel provider managers: \(error)")
            ...
            for (index, tunnelManager) in tunnelManagers.enumerated().reversed() {
                ...
                if let ref = passwordRef {
                    ...
                    print("Removing orphaned tunnel with non-verifying keychain entry: \(tunnelManager.localizedDescription ?? "<unknown>")")
    ...
    func add(tunnelConfiguration: TunnelConfiguration, onDemandOption: ActivateOnDemandOption = .off, completionHandler: @escaping (Result<TunnelContainer, TunnelsManagerError>) -> Void) {
        ...
        tunnelProviderManager.saveToPreferences { [weak self] error in
            if let error = error {
                print("Add: Saving configuration failed: \(error)")
    ...
    func setOnDemandEnabled(_ isOnDemandEnabled: Bool, on tunnel: TunnelContainer, completionHandler: @escaping (TunnelsManagerError?) -> Void) {
        ...
        tunnelProviderManager.saveToPreferences { error in
            if let error = error {
                print("Modify On-Demand: Saving configuration failed: \(error)")
            ...
            if isActivatingOnDemand {
                ...
                tunnelProviderManager.loadFromPreferences { error in
                    ...
                    if let error = error {
                        print("Modify On-Demand: Re-loading after saving configuration failed: \(error)")
                        completionHandler(TunnelsManagerError.systemErrorOnModifyTunnel(systemError: error))
                        return
                    }
                    completionHandler(nil)
                ...
                if tunnelInOperation.isActivateOnDemandEnabled {
                    setOnDemandEnabled(false, on: tunnelInOperation) { [weak self] error in
                        guard error == nil else {
                            print("Unable to activate tunnel '\(tunnel.name)' because on-demand could not be disabled on active tunnel '\(tunnel.name)'")
                            return
                        }
    ...
    private func startObservingTunnelStatuses() {
        statusObservationToken = NotificationCenter.default.observe(name: .NEVPNStatusDidChange, object: nil, queue: OperationQueue.main) { [weak self] statusChangeNotification in
            ...
            print("Tunnel '\(tunnel.name)' connection status changed to '\(tunnel.tunnelProvider.connection.status)'")
            ...
            if tunnel.isAttemptingActivation {
                if session.status == .connected {
                    ...
                    tunnel.isAttemptingActivation = false
                    self.activationDelegate?.tunnelActivationFailed(tunnel: tunnel)
...
class TunnelContainer: NSObject {
    ...
    fileprivate func startActivation(recursionCount: UInt = 0, lastError: Error? = nil, activationDelegate: TunnelsManagerActivationDelegate?) {
        if recursionCount >= 8 {
            print("startActivation: Failed after 8 attempts. Giving up with \(lastError!)")
    ...
    fileprivate func startDeactivation() {
        print("startDeactivation: Tunnel: \(name)")
